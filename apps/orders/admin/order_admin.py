    ...
    def clean(self):
        ...
        for form in self.forms:
            if form.cleaned_data and not form.cleaned_data.get('DELETE', False):
                ...
                print(
                    f"Checking quantity: Product={product.name_uz}, Available={product.quantity}, Requested={quantity}")
    ...
    def save(self, commit=True):
        ...
        if order.type_order == 'cash':
            ...
            if order.order_status == 'pending':
                ...
                # Restore the product quantities if the order is canceled
                for item in order.products.all():
                    product = item.product
                    print(
                        f"Restoring quantity for Product={product.name_uz}: Current={product.quantity}, Adding Back={item.quantity}")
                    product.quantity += item.quantity
                    product.save()
            else:
                # Update product quantities for cash orders
                # Update product quantities
                for item in order.products.all():
                    product = item.product
                    print(
                        f"Reducing quantity for Product={product.name_uz}: Current={product.quantity}, Deducting={item.quantity}")
                    product.quantity -= item.quantity
                    product.save()
                    print(f"New quantity for Product={product.name_uz}: {product.quantity}")

            # Create payment record for cash orders
            for item in order.products.all():
                ...
                print(f"Payment record created for Order={order.pk}, Product={item.product.name_uz}")

        if commit:
            order.save()
            print(f"Order committed: {order.pk}")
        return order
